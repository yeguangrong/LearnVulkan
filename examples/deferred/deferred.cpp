...
class DeferredExample : public VulkanExampleBase
{
public:
	...
	DeferredExample() : VulkanExampleBase(ENABLE_VALIDATION)
	{
	...
	~DeferredExample()
	{
		...
		vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.composition, nullptr);
		...
		FrameBufferAttachment *attachment)
	{
		...
		VK_CHECK_RESULT(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc, nullptr, &attachment->mem));
		VK_CHECK_RESULT(vkBindImageMemory(vulkanDevice->logicalDevice, attachment->image, attachment->mem, 0));
	...
	void prepareOffscreenFramebuffer()
	{
		...
		VK_CHECK_RESULT(vkCreateRenderPass(vulkanDevice->logicalDevice, &renderPassInfo, nullptr, &offScreenFrameBuf.renderPass));
		...
		VK_CHECK_RESULT(vkCreateFramebuffer(vulkanDevice->logicalDevice, &fbufCreateInfo, nullptr, &offScreenFrameBuf.frameBuffer));
	...
	void setupDescriptorSet()
	{
		...
		// Background
		VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice, &allocInfo, &descriptorSets.floor));
		writeDescriptorSets = {
			// Binding 0: Vertex shader uniform buffer
	...
	void preparePipelines()
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.composition));
...
{
	for (int32_t i = 0; i < __argc; i++) { DeferredExample::args.push_back(__argv[i]); };
	deferredExample = new DeferredExample();
