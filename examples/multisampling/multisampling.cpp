...
class MultiSamplingExample : public VulkanExampleBase
{
public:
	...
	MultiSamplingExample() : VulkanExampleBase(ENABLE_VALIDATION)
	{
		title = "Multisampling";
	...
	~MultiSamplingExample()
	{
		...
		vkDestroyPipeline(vulkanDevice->logicalDevice, mPipelines.MSAA, nullptr);
		vkDestroyPipeline(vulkanDevice->logicalDevice, mPipelines.MSAASampleShading, nullptr);

		vkDestroyPipelineLayout(vulkanDevice->logicalDevice, pipelineLayout, nullptr);
		vkDestroyDescriptorSetLayout(vulkanDevice->logicalDevice, descriptorSetLayout, nullptr);

		// Destroy MSAA target
		vkDestroyImage(vulkanDevice->logicalDevice, multisampleTarget.color.image, nullptr);
		vkDestroyImageView(vulkanDevice->logicalDevice, multisampleTarget.color.view, nullptr);
		vkFreeMemory(vulkanDevice->logicalDevice, multisampleTarget.color.memory, nullptr);
		vkDestroyImage(vulkanDevice->logicalDevice, multisampleTarget.depth.image, nullptr);
		vkDestroyImageView(vulkanDevice->logicalDevice, multisampleTarget.depth.view, nullptr);
		vkFreeMemory(vulkanDevice->logicalDevice, multisampleTarget.depth.memory, nullptr);

		uniformBuffer.destroy();
	}

	// Enable physical vulkanDevice->logicalDevice features required for this example
	...
	{
		// Check if vulkanDevice->logicalDevice supports requested sample count for color and depth frame buffer
		...
		info.format = mSwapChain.colorFormat;
		...
		VK_CHECK_RESULT(vkCreateImage(vulkanDevice->logicalDevice, &info, nullptr, &multisampleTarget.color.image));

		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(vulkanDevice->logicalDevice, multisampleTarget.color.image, &memReqs);
		...
		{
			// If this is not available, fall back to vulkanDevice->logicalDevice local memory
			memAlloc.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		}
		VK_CHECK_RESULT(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc, nullptr, &multisampleTarget.color.memory));
		vkBindImageMemory(vulkanDevice->logicalDevice, multisampleTarget.color.image, multisampleTarget.color.memory, 0);

		// Create image view for the MSAA target
		VkImageViewCreateInfo viewInfo = vks::initializers::imageViewCreateInfo();
		viewInfo.image = multisampleTarget.color.image;
		viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
		viewInfo.format = mSwapChain.colorFormat;
		...
		VK_CHECK_RESULT(vkCreateImageView(vulkanDevice->logicalDevice, &viewInfo, nullptr, &multisampleTarget.color.view));
		...
		VK_CHECK_RESULT(vkCreateImage(vulkanDevice->logicalDevice, &info, nullptr, &multisampleTarget.depth.image));

		vkGetImageMemoryRequirements(vulkanDevice->logicalDevice, multisampleTarget.depth.image, &memReqs);
		...
		VK_CHECK_RESULT(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc, nullptr, &multisampleTarget.depth.memory));
		vkBindImageMemory(vulkanDevice->logicalDevice, multisampleTarget.depth.image, multisampleTarget.depth.memory, 0);
		...
		VK_CHECK_RESULT(vkCreateImageView(vulkanDevice->logicalDevice, &viewInfo, nullptr, &multisampleTarget.depth.view));
	}
	...
	void setupRenderPass()
	{
		...
		attachments[2].format = mSwapChain.colorFormat;
	...
	void setupFrameBuffer()
	{
		...
		{
			attachments[2] = mSwapChain.buffers[i].view;
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, useSampleShading ? mPipelines.MSAASampleShading : mPipelines.MSAA);
	...
	void preparePipelines()
	{
		...
		VkGraphicsPipelineCreateInfo pipelineCreateInfo = vks::initializers::pipelineCreateInfo(pipelineLayout, mRenderPass, 0);
		...
		pipelineCreateInfo.pVertexInputState = vkglTF::Vertex::getPipelineVertexInputState({ vkglTF::VertexComponent::Position, vkglTF::VertexComponent::Normal, vkglTF::VertexComponent::UV, vkglTF::VertexComponent::Color });
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCreateInfo, nullptr, &mPipelines.MSAA));
		...
		if (vulkanDevice->features.sampleRateShading)
		{
			...
			VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCreateInfo, nullptr, &mPipelines.MSAASampleShading));
		}
	}
...
MultiSamplingExample* multiSamplingExample;
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (multiSamplingExample != NULL)
	{
		multiSamplingExample->handleMessages(hWnd, uMsg, wParam, lParam);
	}
	return (DefWindowProc(hWnd, uMsg, wParam, lParam));
}
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
	for (int32_t i = 0; i < __argc; i++) { MultiSamplingExample::args.push_back(__argv[i]); };
	multiSamplingExample = new MultiSamplingExample();
	multiSamplingExample->initVulkan();
	multiSamplingExample->setupWindow(hInstance, WndProc);
	multiSamplingExample->prepare();
	multiSamplingExample->renderLoop();
	delete(multiSamplingExample);
	return 0;
}
