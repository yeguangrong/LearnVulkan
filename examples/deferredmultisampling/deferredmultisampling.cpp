...
class DeferredMultiSamplingExample : public VulkanExampleBase
{
public:
	...
	struct {
		VkPipeline deferredMSAA;				// Deferred lighting calculation
	...
	DeferredMultiSamplingExample() : VulkanExampleBase(ENABLE_VALIDATION)
	{
	...
	~DeferredMultiSamplingExample()
	{
		...
		vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.deferredMSAA, nullptr);
		vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.deferredNoMSAA, nullptr);
		vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.offscreen, nullptr);
		vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.offscreenSampleShading, nullptr);
		...
		vkDestroySemaphore(vulkanDevice->logicalDevice, offscreenSemaphore, nullptr);
	}
	...
	void buildDeferredCommandBuffer()
	{
		...
		if (offscreenSemaphore == VK_NULL_HANDLE) {
			...
			VK_CHECK_RESULT(vkCreateSemaphore(vulkanDevice->logicalDevice, &semaphoreCreateInfo, nullptr, &offscreenSemaphore));
		}
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &mDescriptorSet, 0, NULL);

			// Final composition as full screen quad
			// Note: Also used for debug display if debugDisplayTarget > 0
			vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, useMSAA ? pipelines.deferredMSAA : pipelines.deferredNoMSAA);
	...
	void setupDescriptorSet()
	{
		...
		writeDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(mDescriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &texDescriptorPosition),
			// Binding 2: World space normals texture
			vks::initializers::writeDescriptorSet(mDescriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2, &texDescriptorNormal),
			// Binding 3: Albedo texture
			vks::initializers::writeDescriptorSet(mDescriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3, &texDescriptorAlbedo),
			// Binding 4: Fragment shader uniform buffer
			vks::initializers::writeDescriptorSet(mDescriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 4, &uniformBuffers.composition.descriptor),
		};
		...
		// Model
		VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice, &allocInfo, &descriptorSets.model));
		writeDescriptorSets = {
			// Binding 0: Vertex shader uniform buffer
		...
		// Background
		VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice, &allocInfo, &descriptorSets.background));
		writeDescriptorSets = {
			// Binding 0: Vertex shader uniform buffer
	...
	void preparePipelines()
	{
		...
		VkGraphicsPipelineCreateInfo pipelineCI = vks::initializers::pipelineCreateInfo(pipelineLayout, mRenderPass);
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.deferredMSAA));

		// No MSAA (1 sample)
		specializationData = 1;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.deferredNoMSAA));
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.offscreen));

		multisampleState.sampleShadingEnable = VK_TRUE;
		multisampleState.minSampleShading = 0.25f;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkanDevice->logicalDevice, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.offscreenSampleShading));
	}
...
DeferredMultiSamplingExample* deferredExample;
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (deferredExample != NULL)
	{
		deferredExample->handleMessages(hWnd, uMsg, wParam, lParam);
	}
	return (DefWindowProc(hWnd, uMsg, wParam, lParam));
}
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
	for (int32_t i = 0; i < __argc; i++) { DeferredMultiSamplingExample::args.push_back(__argv[i]); };
	deferredExample = new DeferredMultiSamplingExample();
	deferredExample->initVulkan();
	deferredExample->setupWindow(hInstance, WndProc);
	deferredExample->prepare();
	deferredExample->renderLoop();
	delete(deferredExample);
	return 0;
}
