...
public:
	...
	void loadTexture(std::string filename, VkFormat format, bool forceLinearTiling)
	{
		...
		// Get vulkanDevice->logicalDevice properties for the requested texture format
		...
		// Without mip mapping
		VK_CHECK_RESULT(vkCreateSampler(vulkanDevice->logicalDevice, &sampler, nullptr, &samplers[0]));

		// With mip mapping
		sampler.maxLod = (float)texture.mipLevels;
		VK_CHECK_RESULT(vkCreateSampler(vulkanDevice->logicalDevice, &sampler, nullptr, &samplers[1]));
		...
		VK_CHECK_RESULT(vkCreateSampler(vulkanDevice->logicalDevice, &sampler, nullptr, &samplers[2]));
